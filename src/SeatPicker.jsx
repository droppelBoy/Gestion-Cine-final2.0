...
const SeatPicker = ({ pelicula, onConfirmarReserva, onCancelar }) => {
  ...
  const total = seleccionados.length * (pelicula.precio || 4500);

  return (
    <div className="card bg-dark text-light border-secondary p-4 mt-4 shadow-lg">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-bold m-0 text-danger">Selección de Asientos - {pelicula.titulo}</h4>
      ...
      <div className="text-center my-3">
        <div 
          ...
          style={{ width: '80%', height: '8px', boxShadow: '0 0 15px rgba(255,255,255,0.6)' }}
        ></div>
      ...
      <div className="d-flex flex-column align-items-center gap-2 my-4">
        {filas.map((fila) => (
          <div key={fila} className="d-flex align-items-center gap-2">
            <span className="fw-bold text-secondary me-2" style={{ width: '20px' }}>{fila}</span>
            {columnas.map((col) => {
              ...
              return (
                <button
                  key={asientoId}
                  className={btnClass}
                  style={{ width: '38px', height: '38px', fontSize: '0.75rem', padding: 0 }}
      ...
      <div className="d-flex justify-content-center gap-4 text-secondary small mb-3">
        <div><span className="badge bg-outline-secondary border border-secondary text-light me-1">○</span> Disponible</div>
      ...
      <div className="border-top border-secondary pt-3 d-flex justify-content-between align-items-center">
        <div>
          <div>Asientos seleccionados: <strong className="text-info">{seleccionados.join(', ') || 'Ninguno'}</strong></div>
          <div className="fs-5">Total: <strong className="text-warning">${total.toLocaleString('es-CL')}</strong></div>
        </div>
        <button
